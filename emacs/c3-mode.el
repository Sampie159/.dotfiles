  ...
  (let ((cur-line (string-trim-right (thing-at-point 'line t)))
    ...
    (cond
     ...
     ((or (string-suffix-p "{" prev-line)
          ...
          (string-prefix-p "$switch" (string-trim-left prev-line))
          (string-suffix-p "$else" prev-line))
      (+ (c3--space-prefix-len prev-line) indent-len))
     ((string-prefix-p "$endswitch" (string-trim-left cur-line))
      (max (- (c3--space-prefix-len prev-line) (* 2 indent-len)) 0))
     ((or (string-prefix-p "}" (string-trim-left cur-line))
          (string-prefix-p "$end" (string-trim-left cur-line))
          (string-prefix-p "$case" (string-trim-left cur-line))
          (string-prefix-p "$default" (string-trim-left cur-line))
          (string-prefix-p "$else" (string-trim-left cur-line)))
